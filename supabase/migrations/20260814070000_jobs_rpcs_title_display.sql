-- title_display stage 2 (design 2026-08-14): serve the cleaned display title
-- on the RPC-backed jobs list paths. Stage 1 (20260814060000) added the
-- column + ingest writes + the direct-query projections; these two functions
-- are the remaining readers.
--
-- RETURNS TABLE changes, so CREATE OR REPLACE cannot be used — DROP + CREATE.
-- Definitions are byte-faithful to prod's current functions (captured via
-- pg_get_functiondef against a fully-migrated database) with exactly two
-- insertions each: `title_display text` in RETURNS (after `title`) and
-- `jp.title_display` in the SELECT at the same position (RETURN QUERY matches
-- by POSITION, so the two lists must stay aligned).
--
-- Callers read rows as JSON objects (PostgREST rpc), so the added key is
-- invisible to an older API — and a newer API tolerates its absence
-- (FE falls back to raw title). Order-independent either way.
...
CREATE FUNCTION public.get_target_jobs(
    ...
    p_after_value text DEFAULT NULL::text,
    p_after_id uuid DEFAULT NULL::uuid,
    p_user_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(
    id uuid, external_id text, source_id uuid, title text, title_display text,
    company_name text, location text, city text, state text, country text,
    location_remote boolean, employment_type text, seniority text, metro text,
    is_remote boolean, absolute_url text, score integer, score_breakdown jsonb,
    ...
    salary_period text, source_posted_at timestamp with time zone,
...
RETURNS TABLE(
    ...
    company_name text, location text, city text, state text, country text,
    location_remote boolean, employment_type text, seniority text, metro text,
    is_remote boolean, absolute_url text, score integer, raw_score integer,
    score_breakdown jsonb, scoring_status text, logistics_filters jsonb,
    status text, salary_text text, salary_min numeric, salary_max numeric,
    salary_currency text, salary_period text,
    source_posted_at timestamp with time zone,
    cataloged_at timestamp with time zone, pending boolean)
LANGUAGE plpgsql
STABLE
SET search_path TO ''
AS $function$
