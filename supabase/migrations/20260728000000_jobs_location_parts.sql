-- #518 location normalization: structured location parts on jobs.
--
-- ``parse_location`` (app/services/location_parse.py) derives
-- (city, state, country, location_remote) from the raw ATS ``location``
-- string at ingest; the poller re-derives every cycle (#514), so the live
-- corpus converges organically — same model as salary_text. The raw
-- ``location`` column stays untouched as the display fallback for
-- unparseable strings ("2 Locations", "Hybrid", campus names).
--
-- ``location_remote`` is board-STATED remoteness (explicit "Remote" wording
-- in the location string) — deliberately separate from the LLM tagger's
-- JD-inferred ``is_remote``.
--
...
ALTER TABLE public.jobs
  ...
  ADD COLUMN IF NOT EXISTS location_remote boolean;

-- ---------------------------------------------------------------------------
-- get_target_jobs: verbatim 20260708040000 definition + the four columns.
-- ---------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.get_target_jobs(p_target_id uuid, p_min_score integer DEFAULT 0, p_status text DEFAULT NULL::text, p_company text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_sort text DEFAULT 'score'::text, p_ascending boolean DEFAULT false, p_limit integer DEFAULT 20, p_after_value text DEFAULT NULL::text, p_after_id uuid DEFAULT NULL::uuid, p_user_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, external_id text, source_id uuid, title text, company_name text, location text, city text, state text, country text, location_remote boolean, department text, absolute_url text, score integer, score_breakdown jsonb, scoring_status text, logistics_filters jsonb, status text, salary_text text, greenhouse_updated_at timestamp with time zone, first_seen_at timestamp with time zone, created_at timestamp with time zone)
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
AS $function$
...
-- ---------------------------------------------------------------------------
-- get_cross_target_jobs: verbatim 20260723000000 definition + the four columns.
-- ---------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.get_cross_target_jobs(p_target_ids uuid[], p_min_score integer DEFAULT 0, p_status text DEFAULT NULL::text, p_company text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_sort text DEFAULT 'score'::text, p_ascending boolean DEFAULT false, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_user_id uuid DEFAULT NULL::uuid, p_recency_decay boolean DEFAULT false, p_weights jsonb DEFAULT '{}'::jsonb)
 RETURNS TABLE(id uuid, external_id text, source_id uuid, title text, company_name text, location text, city text, state text, country text, location_remote boolean, department text, absolute_url text, score integer, raw_score integer, score_breakdown jsonb, scoring_status text, logistics_filters jsonb, status text, salary_text text, greenhouse_updated_at timestamp with time zone, first_seen_at timestamp with time zone, created_at timestamp with time zone, pending boolean)
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
AS $function$
