-- Family gate: tolerate UNKNOWN (2026-08-15).
--
-- UNKNOWN-TOLERANT. ``role_family = 'other'`` is what the tagger emits when
--    it cannot classify a posting — and what the model-output coercion
--    substitutes for a malformed value. It means "don't know", exactly like
--    NULL. The old rule passed NULL but EXCLUDED 'other', so the same
--    uncertainty was treated two opposite ways and 1,852 live postings (11.1%
--    of the catalog, measured today) were hidden from every classified target.
--    A consistency fix: it only affects jobs whose family is UNKNOWN, never
--    jobs known to be in a different discipline.
--
-- ADJACENCY between known families is deliberately NOT included. It was planned
-- as mitigation for a role_family regression caused by folding skill extraction
-- into the tagger prompt; that extraction now runs as a zero-cost dictionary, so
-- the regression never happens and the mitigation is unnecessary. Widening what
-- every target matches is a product decision that deserves its own evidence.
--
-- The rule now has ONE definition per language: ``family_gate_passes`` here,
-- ``app/services/qualification/family_gate.py`` in Python. Both RPCs call this
-- function instead of inlining the comparison, so the previous "change both or
-- neither" drift risk ACROSS TWO RPCs is gone. tests/test_family_gate.py pins
-- the Python/SQL parity case-for-case.
--
-- behavioral, no schema change: the RPCs return strictly MORE rows than before,
-- so this is safe to apply before the code deploys.

CREATE OR REPLACE FUNCTION public.family_gate_passes(
...
AS $function$
    SELECT
        -- Unclassified target: ungated.
        p_target_family IS NULL
        -- Unknown job family (untagged, or the tagger's catch-all): benefit
        -- of the doubt, same spirit as ``is_us IS NOT FALSE``.
        OR p_job_family IS NULL
        OR p_job_family = 'other'
        -- Otherwise exact match only: the gate still keeps off-discipline
        -- postings out of a target's matches.
        OR p_job_family = p_target_family;
$function$;

GRANT ALL ON FUNCTION public.family_gate_passes(text, text)
    TO anon, authenticated, service_role;


-- Both RPCs below are reproduced VERBATIM from their live definitions, with
-- ONLY the family predicate swapped to call family_gate_passes.
