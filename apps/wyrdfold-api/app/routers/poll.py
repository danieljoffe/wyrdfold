...
async def trigger_poll() -> dict[str, str]:
    ...
    detached from the request, so manual/external triggers stop hitting the
    edge's 300s timeout — the full poll used to run synchronously inside the
    request, so a curl caller got a 499/502 timeout even when the poll
    itself was fine.

    The poll is routed through ``run_force_poll_locked``, which takes the
    SAME advisory lock as the scheduler: a manual trigger and the scheduled
    due-poll can't run concurrently, and a trigger fired while a poll is
    already running logs "poll already running, skipping" and exits cleanly.
    The body wraps the poll in try/except so a failure is logged rather than
    silently swallowed.

    Spawned via :func:`app.background.spawn_detached`, NOT starlette
    ``BackgroundTasks`` — with ``POLLER_ASYNC_DB`` on, the poll's async DB
    fan-out deadlocks the pooled httpx client when run inside the request's
    background machinery under uvloop (see ``app/background.py``).
    ...
    """
    spawn_detached(run_force_poll_locked(), name="force-poll")
