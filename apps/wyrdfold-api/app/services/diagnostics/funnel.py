...
from app.services.fit.daily_cap import phase2_quota_remaining, phase2_quota_remaining_async
...
# ── Async twins (#57 PR-G2c) ─────────────────────────────────────────────────
# The interactive funnel handler is now ``async def`` on the pooled async
# service client, so it awaits ``compute_target_funnel_async``. The sync
# ``compute_target_funnel`` (+ its read helpers) stays for the diagnostic
# ``scripts/diagnose_target_funnel.py`` + the unit test, which hold a sync
# client. Each twin runs the SAME query as its sync counterpart, awaited; the
# pure derivations (``_stage_counts`` / ``_histogram`` / ``_default_floor_from_users``)
# and the ``crud._parse_target`` row parse are reused verbatim so the response
# shape stays byte-identical. ``crud.get`` is inlined as an awaited read (crud
# stays sync for its poller/pipeline callers).


async def _fetch_score_rows_async(supabase: AsyncClient, target_id: str) -> list[dict[str, Any]]:
    """Async mirror of :func:`_fetch_score_rows`."""
    resp = await (
        supabase.table("scores")
        .select("promising, scoring_status, excluded, score")
        .eq("target_id", target_id)
...
async def _user_context_async(supabase: AsyncClient, target_id: str) -> list[FunnelUserContext]:
    """Async mirror of :func:`_user_context` — batched profile read + the
    target-scoped quota fetched once (async), preserving row order."""
    ut_resp = await (
        supabase.table("user_targets")
        .select("user_id, is_active")
        .eq("target_id", target_id)
    ...
    quota = await phase2_quota_remaining_async(supabase, target_id) if user_rows else 0
...
async def _sources_async(supabase: AsyncClient) -> list[FunnelSourceStaleness]:
    """Async mirror of :func:`_sources`."""
    resp = await (
        supabase.table("sources")
        .select("id, company_name, provider, enabled, last_polled_at, job_count")
        .order("last_polled_at", desc=True, nullsfirst=True)
...
async def compute_target_funnel_async(
    supabase: AsyncClient, target_id: str
) -> TargetFunnelResponse:
    """Async mirror of :func:`compute_target_funnel` (#57 PR-G2c).

    Same read-only report, awaited on the pooled async service client for the
    ``async def`` funnel handler. Raises ``ValueError`` when the target is
    missing (the handler maps it to 404), identical to the sync version."""
    resp = await supabase.table(crud.TARGETS_TABLE).select("*").eq("id", target_id).execute()
    target_rows = cast(list[dict[str, Any]], resp.data or [])
    if not target_rows:
        raise ValueError(f"Target {target_id!r} not found")
    ...
    # One scan of `scores` feeds both the stage counts and the histogram.
    score_rows = await _fetch_score_rows_async(supabase, target_id)
    stages = _stage_counts(score_rows)
    # User context first — we need their floor for the histogram view.
    users = await _user_context_async(supabase, target_id)
    floor = _default_floor_from_users(users)
    histogram = _histogram(score_rows, floor=floor)
    sources = await _sources_async(supabase)
