...
# Résumé-free variant (#78 layer 1). Same schema + keyword/title/slim rules
# as SYSTEM_PROMPT, but the scoring_profile is grounded in role-generic
# industry knowledge rather than one user's experience — so a shared target
# isn't skewed by whoever activated it. Gated by
# ``settings.resume_free_label_derivation``; the résumé only feeds fit_score.
...
async def derive_profile_from_label(
    ...
    *,
    ...
    Two modes (see ``settings.resume_free_label_derivation``, #78 layer 1):

    * **résumé-free** (flag on, or ``payload is None``) — the profile is
      grounded in role-generic industry knowledge from the label alone, so
      a shared target isn't skewed by whoever activated it.
    * **résumé-grounded** (default) — the profile is grounded in the user's
      experience (``payload``), the historical behavior.

    Either way the résumé feeds ``fit_score`` separately; this only governs
    the shared scoring rubric. Returns (derived, result) so callers can log
    cost.
    """
    from app.config import settings

    if settings.resume_free_label_derivation or payload is None:
