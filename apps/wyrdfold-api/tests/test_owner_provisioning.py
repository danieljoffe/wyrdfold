...
def test_self_host_with_owner_email_creates_the_user() -> None:
    sb = MagicMock()
    provision_owner(sb, _settings())
    sb.auth.admin.create_user.assert_called_once_with(
        {"email": "owner@example.com", "email_confirm": True}
    )


def test_email_is_normalized_before_create() -> None:
    sb = MagicMock()
    provision_owner(sb, _settings(owner_email="  Owner@Example.COM "))
    sb.auth.admin.create_user.assert_called_once_with(
        {"email": "owner@example.com", "email_confirm": True}
    )


def test_saas_mode_never_provisions() -> None:
    sb = MagicMock()
    provision_owner(sb, _settings(deployment_mode="saas"))
    sb.auth.admin.create_user.assert_not_called()


def test_unset_owner_email_is_a_noop() -> None:
    sb = MagicMock()
    provision_owner(sb, _settings(owner_email=""))
    sb.auth.admin.create_user.assert_not_called()


def test_already_registered_is_idempotent_success(
...
def test_other_failures_warn_and_do_not_raise(
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    with caplog.at_level("WARNING"):
        provision_owner(sb, _settings())  # must not raise
    assert any(
