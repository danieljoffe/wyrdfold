...
class _FakeBucket:
    ...
    async def upload(self, *, path: str, file: bytes, file_options: dict) -> None:
...
async def test_storage_path_is_namespaced_under_user_id() -> None:
    ...
    path = await persistence.upload_docx(sb, user_id="user-1", resume_id="r-1", docx_bytes=b"y")

    assert path == "user-1/r-1.docx"


async def test_two_users_same_resume_id_do_not_collide() -> None:
    ...
    assert await persistence.download_docx(sb, p2) == b"two"


async def test_download_unknown_path_raises() -> None:
    ...
    with pytest.raises(KeyError):
        await persistence.download_docx(sb, "user-1/never-written.docx")


async def test_upload_sets_docx_content_type() -> None:
    ...
    await persistence.upload_docx(sb, user_id="u", resume_id="r", docx_bytes=b"z")
