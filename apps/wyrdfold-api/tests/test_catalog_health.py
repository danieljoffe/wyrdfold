...
class _FakeQuery:
    ...
    queued result for its table. ``insert``/``delete`` payloads are captured
    ...
    async def execute(self) -> SimpleNamespace:
        return SimpleNamespace(data=self._owner.next_result(self._table))


class _FakeSupabase:
    """Queue-per-table fake: ``results[table]`` is consumed in call order,
    covering the recorder's fixed query sequence (throttle → jobs window →
    baseline → scores chunks → rpc → insert → delete)."""
...
def test_tripwire_fires_on_a_regime_change() -> None:
    ...
    fired, distance, reason = evaluate_tripwire(
        _ASSISTANT_WINDOW, _ENGINEER_BASELINE, threshold=0.5, min_sample=20
    )
...
def test_tripwire_quiet_on_a_stable_mix() -> None:
    ...
    fired, distance, reason = evaluate_tripwire(
        window, _ENGINEER_BASELINE, threshold=0.5, min_sample=20
    )
...
def test_tripwire_refuses_a_small_window() -> None:
    fired, distance, reason = evaluate_tripwire(
        Counter({"assistant": 3}), _ENGINEER_BASELINE, threshold=0.5, min_sample=20
    )
...
def test_tripwire_refuses_a_small_baseline() -> None:
    fired, distance, reason = evaluate_tripwire(
        _ASSISTANT_WINDOW, Counter({"engineer": 2}), threshold=0.5, min_sample=20
    )
    assert (fired, distance) == (False, None)
