...
def test_us_does_not_match_austin() -> None:
    ...
    assert _location_passes("Austin", exclude_terms=[], only_terms=["us"]) is False


def test_us_does_not_match_belarus() -> None:
    """Belarus has 'us' as a substring but is NOT in the United States."""
    assert _location_passes("Minsk, Belarus", exclude_terms=[], only_terms=["us"]) is False


def test_us_does_not_match_mauritius() -> None:
    assert _location_passes("Port Louis, Mauritius", exclude_terms=[], only_terms=["us"]) is False
...
def test_us_matches_usa_synonym() -> None:
    assert _location_passes("-REMOTE, USA-", exclude_terms=[], only_terms=["us"])
...
def test_us_matches_us_with_punctuation() -> None:
    ...
    assert _location_passes("Remote, U.S.", exclude_terms=[], only_terms=["us"])


def test_us_matches_us_dash_state() -> None:
    """'US-CA-Menlo Park' format — dash separator, 'US' as the leading
    bounded token."""
    assert _location_passes("US-CA-Menlo Park", exclude_terms=[], only_terms=["us"])
...
def test_uk_matches_united_kingdom() -> None:
    assert _location_passes("London, United Kingdom", exclude_terms=[], only_terms=["uk"])


def test_uk_does_not_match_truck() -> None:
    """'uk' substring matches in 'truck' — word-boundary catches this."""
    assert _location_passes("Truckee, CA", exclude_terms=[], only_terms=["uk"]) is False
...
def test_brazil_substring_matches_sao_paulo_brazil() -> None:
    assert _location_passes("São Paulo, Brazil", exclude_terms=[], only_terms=["brazil"])
...
def test_exclude_us_drops_remote_us() -> None:
    assert _location_passes("Remote, US", exclude_terms=["us"], only_terms=[]) is False
...
def test_missing_location_drops_when_only_terms_set() -> None:
    ...
    assert _location_passes(None, exclude_terms=[], only_terms=["us"]) is False
