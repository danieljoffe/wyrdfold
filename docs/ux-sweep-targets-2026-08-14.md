...
## F. Disposition — what shipped, what did not

Six PRs into `develop`, in the order they landed.

| PR   | Findings | What changed                                                                                                                                                                                                                                                      |
| ---- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| #742 | A1       | Durable empty state for both suggest actions (the toast was the _only_ signal for a 20–48s call, and it auto-dismisses in 4s). Also gave the zero-state button in-flight feedback.                                                                                |
| #743 | A2, B2   | `/jobs` names a paused target and offers to resume it instead of silently redirecting to another target's jobs. Detail header renders **Inactive** (not nothing) and gains the activate toggle.                                                                   |
| #744 | A6       | The active-target-cap 409 surfaces the server's own sentence instead of `Activate failed (409)`. Three duplicated raise sites folded into one builder; `1 active targets` pluralization fixed.                                                                    |
| #745 | A4       | Posting titles are canonicalized into reusable role labels **before** matching, so the canonical form is also the dedup key. Non-fatal — a normalizer failure degrades to today's raw title. Both label-canonicalization prompts pinned in the regression golden. |
| #746 | A5       | A failed create re-opens the modal holding the draft, on the tab it failed from. The extraction error's recovery hint is now caller-specific, so create-from-URL stops advising a JD-paste field that does not exist there.                                       |
| #747 | B1, B3   | Delete prompt names the target. Score bounds cut from 200 to the real 100, with a `mode="before"` clamp so legacy rows still read.                                                                                                                                |

### Not fixed — and why

| Finding                                                                                                                                                | Status                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **A3** (three creation paths land Inactive with no prompt)                                                                                             | **Partly addressed, deliberately not completed.** #743 makes the state legible everywhere it was invisible. The remaining piece — a post-creation "activate this to start matching" nudge — is gated on A6's cap: with `MAX_ACTIVE_TARGETS_PER_USER = 1` by default, prompting every new target to activate walks most users straight into a 409. Worth designing against the cap (e.g. offer the swap), not bolting on. |
| **B4** (fit reasoning trapped in a `title` tooltip; five names for one number)                                                                         | Open. The `title`/`aria-label` disagreement is one line (`ScoreBadge.tsx:68-69`); the detail header's chip is a plain `Badge` with no tooltip at all. Both are small, but "what should this number be called" is a product decision, not a refactor.                                                                                                                                                                     |
| **B5** (one target permanently stuck with no fit score)                                                                                                | Open. Needs a recompute affordance — there is none anywhere — plus a look at _why_ that row never scored. One stuck record, not a path-wide bug (see R3).                                                                                                                                                                                                                                                                |
| **B6** (dates render in UTC)                                                                                                                           | Open, root cause identified: `LocalDate`'s `suppressHydrationWarning` makes React keep the server's UTC text forever. The fix is a mount-gated re-render, and `LocalFormat.spec.tsx:34-35` currently pins the buggy contract, so it changes with it. Affects every `LocalDate` caller, so it deserves its own PR.                                                                                                        |
| **C1–C6** (Cancel-after-success wording, undismissable panel, `?tab=` fallback, cards aren't links, redundant `/user-target` fetches, fit-score drift) | Open. C6 (a fit score moving 95→82 untouched) is the one worth a real look — it is a correctness question, not polish.                                                                                                                                                                                                                                                                                                   |
| **Create-from-pasted-JD**                                                                                                                              | Not built. `from_url`'s fetch is entangled with source registration and job materialization, neither of which a pasted JD has, so this is a new endpoint and a feature — not a fix to smuggle into #746.                                                                                                                                                                                                                 |

### Owner calls outstanding

- **#745 is not eval-verified.** The prompt-regression guard is spend-free and pins _inputs_; it does not measure quality. A spend-bearing eval of the new title normalizer is a release decision (`CONTRIBUTING.md` → "Touching prompts or scoring code").
- **#745 adds one inline LLM call** to create-from-URL, which previously had none. Correct dedup requires it inline; the latency cost is real and accepted.
